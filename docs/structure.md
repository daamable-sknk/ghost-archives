...
├── dashboard.html              # 대시보드 — 파이프라인 상태, 카테고리 분포, 수집 추이
...
| reviewed | boolean | 수동 검토 완료 여부 (미사용, 향후 정책 결정 필요) |
...
| 페이지 | 파일 | 내용 |
|--------|------|------|
| 목록 | index.html | 수집 데이터 목록, 소스/언어 필터, 분류 건수 |
| 대시보드 | dashboard.html | 파이프라인 상태(수집→분류→미분류), 카테고리 분포 차트, 주간 수집 추이 (Chart.js) |
